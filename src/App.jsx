...
// ─── ROOMS ─────────────────────────────────────────────────────────────────────
const ROOMS = [
  { id:'dash',    label:'Command',    sub:'Unit Z649 Overview',    glyph:'CM', accent:'#5B8CFF', cam:'rotateX(52deg) translate3d(0,0,-120px)' },
  ...
  { id:'fin',     label:'Treasury',   sub:'Financial Command',     glyph:'TR', accent:'#FF5D8F', cam:'rotateX(44deg) rotateY(22deg) translate3d(0,-460px,40px)' },
...
const findAI = (q, crew, vault, finances, news) => {
  ...
  return 'That\'s a great question. For detailed answers, I work best with specific topics — try asking about: NASA India, LIK Trophy, MSL Trophy, vernacular architecture, Le Corbusier, parametric design, passive cooling, heritage conservation, AutoCAD, Revit, or your unit\'s treasury and crew. What would you like to dive into?';
};
...
export default function App() {
  ...
  const [expArchive,  setExpArchive]  = useState([]);
  ...
  const archiveCampaign = async () => {
    ...
    // Reset contributions for new tenure
  ...
  const renderModal = () => {
    ...
    if(T==='add-news'||T==='edit-news') return <>
      ...
      <p style={{fontSize:'var(--fs1)',color:'var(--ink3)',margin:'-.5rem 0 1rem 0'}}>All registered members will receive an email notification.</p>
  ...
  const renderAI = () => <div className="wrap">
    ...
    <p className="lede reveal d2">Comprehensive knowledge on NASA India, all trophies, architecture theory, and unit management.</p>
    ...
    <div className="chips reveal d4">
      {['help','What is the LIK Trophy?','Tell me about NASA India','Explain vernacular architecture','Who is Le Corbusier?','balance','clear'].map(cmd=><button key={cmd} className="chip" onClick={()=>setTermIn(cmd)}>{cmd}</button>)}
    </div>
  </div>;
